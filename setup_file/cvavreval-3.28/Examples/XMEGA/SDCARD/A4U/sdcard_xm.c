...
/* ATxmega128A4U I/O register definitions */
#include <io.h>
...
if ((res=f_open(&file,file_name,FA_READ))==FR_OK)
...
if ((res=f_mkdir(directory))==FR_OK)
   printf("Directory \"%s\" created OK\r\n",directory);
else
if (res==FR_EXIST)
...
if ((res=f_chdir(directory))==FR_OK)
...
/* open the file in read mode */
if ((res=f_open(&file,file_name,FA_WRITE))==FR_OK)
   printf("File \"%s\" opened OK\r\n",file_name);
else
   /* an error occurred, display it and stop */
   error(res);
...
/* display file's attribute, size and time stamp */
display_status(file_name);

/* change file's attributes, set the file to be Read-Only */
if ((res=f_chmod(file_name,AM_RDO,AM_RDO))==FR_OK)
...
if ((res=f_utime(file_name,&finfo))==FR_OK)
...
if ((res=f_chmod(file_name,0,AM_RDO))==FR_OK)
